...
public class ReservationsHandler {
	private final String ReservationsTable = "cmtr-ecf0d511-Reservations-test";
	private final String TablesTable = "cmtr-ecf0d511-Tables-test";
	...
	public APIGatewayProxyResponseEvent postReservations(APIGatewayProxyRequestEvent request) {
		...
		DynamoDBQueryExpression<Tables> queryExpression = new DynamoDBQueryExpression<Tables>()
				.withIndexName("TableIndex").withConsistentRead(false).withKeyConditionExpression("#num = :val")
				.withExpressionAttributeNames(Collections.singletonMap("#num", "number"))
				.withExpressionAttributeValues(Collections.singletonMap(":val",
						new AttributeValue().withN(String.valueOf(reservationItem.getTableNumber()))));
		...
		ScanResult scanResult = client.scan(new ScanRequest().withTableName(ReservationsTable)); // Replace with the
																									// name of your
																									// 'Reservations'
																									// table
