...
import com.task06.util.AuditInsert;
import com.task06.util.AuditUpdate;
import com.task06.util.ConfigurationItem;
...
public class AuditProducer implements RequestHandler<DynamodbEvent, Void> {
	...
	private void handleInsertEvent(DynamodbEvent.DynamodbStreamRecord record) {
		...
		logger.info("audit insert here " + auditUpdate);
	...
	private void handleModifyEvent(DynamodbEvent.DynamodbStreamRecord record) {
		...
		logger.info("audit update here"+auditUpdate);
		AmazonDynamoDB dynamoDBClient = AmazonDynamoDBClientBuilder.defaultClient();
		PutItemRequest putItemRequest = new PutItemRequest()
				.withTableName(table_name)
				.withItem(auditUpdate.toMap());
